...
def create_story(
    ...
    db.add(job)
    db.commit()
    # Adding background tasks, generate story
...
def build_complete_story_tree(db: Session, story: Story) -> CompleteStoryResponse:
    ...
    for node in nodes:
        # print(f'Processing node {node.id} with content: {node.content}')
    ...
    print(f'Root node: {root_node.content}')
    if not root_node:
